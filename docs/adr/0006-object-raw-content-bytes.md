...
- **对象编码按块粒度**：assistant 消息天然多块（thinking + text + 多个 tool_call），块粒度免去消息级的分隔/转义发明；各块编码：
  ...
  - image → 图片字节本体（mime_type 入 `entry_blocks` 列）；
  - tool_call → arguments 的 canonical JSON（id / name 入 `entry_blocks` 列）；
...
- **出处双轨 → 仅结构轨（对 ADR-0001 的修订）**：删除 `SummarySegment.sources`；原文取回唯一走 `fetch_originals`。理由：块粒度后消息无单一哈希，sources 无直接后继；生产路径仅 `compact()`，结构轨全覆盖；顺带消除长会话哈希膨胀（`docs/research/compaction.md` R6）。
...
- `append_segment` 返回语义从消息哈希变为坐标 `"<session_id>:<seq>"`（块粒度后消息无单一哈希；签名不变，`materialize` 同）。
- 未合并的 `feat/compaction` 分支 rebase 到本决策后须同步删除 `compaction_coordinator` 的 sources 构造链（master 上不存在该链），否则编译不过。
